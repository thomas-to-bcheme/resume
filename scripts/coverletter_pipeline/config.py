"""Paths, constants, layout configuration, and type aliases for cover letters.
...
DEFAULT_OUTPUT_NAME: str = "Thomas_To_CoverLetter"
...
CHAR_BUDGET_MIN: int = 1500
CHAR_BUDGET_MAX: int = 2500
WORD_COUNT_MIN: int = 250
WORD_COUNT_MAX: int = 400
...
class LayoutConfig:
    """Adjustable layout parameters for single-page cover letter PDF.

    Cover letters are short enough (250-400 words) that page-fit optimization
    is not needed. This config provides baseline layout values.
    """

    font_size_body: float = 10.5
    line_height_mult: float = 1.25
    paragraph_space_before: float = 6 / 72    # inches (converted from points)
    date_space_before: float = 12 / 72
    date_space_after: float = 6 / 72
    greeting_space_before: float = 6 / 72
    greeting_space_after: float = 6 / 72
    signature_space_before: float = 12 / 72
